...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if new_PgNode_a.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(new_PgNode_a)
                for node in self.s_levels[level]:
                    new_PgNode_a.parents.add(node)
                    new_PgNode_a.children.add(node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_eff_add in node_a1_action.effect_add:
            if a1_eff_add in node_a2_action.effect_rem:
                return True
        for a2_eff_add in node_a2_action.effect_add:
            if a2_eff_add in node_a1_action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect in node_a2.action.effect_rem:
            if effect in node_a1.action.precond_pos:
                return True
        # precond of a node oposite of effect of other node should be mutex
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pr1 in node_a1.parents:
            for pr2 in node_a2.parents:
                if pr1.is_mutex(pr2) or  pr2.is_mutex(pr1):
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for s1_parent in node_s1.parents:
            for s2_parent in node_s2.parents:
                if not s1_parent.is_mutex(s2_parent) and not s2_parent.is_mutex(s1_parent):
